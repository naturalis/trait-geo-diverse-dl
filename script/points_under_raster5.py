...
#file 2 could not process lama lama guanico (row 79) and subsequently row 80. Therefore separately in this file
file_dir=r'C:/Users/M-RAM/Documents/InternshipNaturalis/trait-geo-diverse-ungulates-master'

#access file with list of taxa names
taxa5=pd.read_csv(file_dir+"/data/filtered/taxa.txt",header=None)
taxa5.columns=["taxon"]
taxa5=taxa5[78:80]
print(taxa5["taxon"])
...
for i in taxa5["taxon"]:
    data = pd.read_csv(file_dir+"/species_occ/%s_occ_dataframe.csv"%i)
    spec = data["taxon_name"][0]
    spec = spec.replace(" ","_")
    print(spec)
    src = rasterio.open(file_dir + "/data/GIS/ENVIREM_BIOCLIM_stacked.tif")
